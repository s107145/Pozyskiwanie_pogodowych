...
def save_current_data(new_data: dict):
    ...
    if not new_data:
        logger.warning("Brak nowych danych — nie zapisano.")
        return
    ...
    try:
        ...
        logger.info(f"Zapisano aktualne dane do {CURRENT_FILE}")

    except Exception as e:
        logger.error(f"Błąd przy zapisie danych: {e}")
...
def run_current_monitoring(frequency_sec: int, duration_sec: int = None):
    """
    Monitorowanie danych aktualnych z możliwością ograniczenia czasu działania.
    """
        ...
        while True:
            ...
            if current_data:
                ...
                for m in measurements:
                    ...
                    unit = m.get("unit") or m.get("parameter", {}).get("units")
                    value = m.get("value")

                    logger.debug(f"Pomiary: {param} = {value} {unit} | {dt}")
